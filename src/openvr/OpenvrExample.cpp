...
class OpenvrExample: public Platform::Application {
    public:
        explicit OpenvrExample(const Arguments& arguments);

    private:
        void drawEvent() override;
        void keyPressEvent(KeyEvent& event) override;

        OpenvrIntegration::VRSystem _vrSystem;
//        std::unique_ptr<OpenvrIntegration::Session> _session;
...
//        OpenvrIntegration::LayerEyeFov* _layer;
//        OpenvrIntegration::PerformanceHudMode _curPerfHudMode{
//            OpenvrIntegration::PerformanceHudMode::Off};
//        OpenvrIntegration::DebugHudStereoMode _curDebugHudStereoMode{
//            OpenvrIntegration::DebugHudStereoMode::Off};
        ...
        GL::Texture2D _color[2]{GL::Texture2D{NoCreate},
                                GL::Texture2D{NoCreate}};
//        std::unique_ptr<OpenvrIntegration::TextureSwapChain> _textureSwapChain[2];
...
OpenvrExample::OpenvrExample(const Arguments& arguments): Platform::Application(arguments, NoCreate) {

    /* Get the HMD display resolution */
    const Vector2i resolution = _vrSystem->get_recommended_render_target_size();

    /* Create a context with the HMD display resolution */
    Configuration conf;
    conf.setTitle("Magnum OpenVR Example")
        .setSize(resolution);
...
//           .setHighQuality(true);
    ...
    /* Setup per-eye views */
    for(OpenvrIntegration::VREye eye: {OpenvrIntegration::VREye ::LeftEye, OpenvrIntegration::VREye ::RightEye}) {
        int eye_idx = static_cast<int>(eye);
        _projectionMatrix[eye_idx] = _vrSystem.get_proj_matrix(eye, 0.001f, 100.0f);
...
void OpenvrExample::keyPressEvent(KeyEvent& event) {
    /* Toggle through the performance hud modes */
    if(event.key() == KeyEvent::Key::F11) {
        switch(_curPerfHudMode) {
            case OpenvrIntegration::PerformanceHudMode::Off:
                _curPerfHudMode = OpenvrIntegration::PerformanceHudMode::LatencyTiming;
                break;
            case OpenvrIntegration::PerformanceHudMode::LatencyTiming:
                _curPerfHudMode = OpenvrIntegration::PerformanceHudMode::AppRenderTiming;
                break;
            case OpenvrIntegration::PerformanceHudMode::AppRenderTiming:
                _curPerfHudMode = OpenvrIntegration::PerformanceHudMode::CompRenderTiming;
                break;
            case OpenvrIntegration::PerformanceHudMode::CompRenderTiming:
                _curPerfHudMode = OpenvrIntegration::PerformanceHudMode::PerfSummary;
                break;
            case OpenvrIntegration::PerformanceHudMode::PerfSummary:
                _curPerfHudMode = OpenvrIntegration::PerformanceHudMode::VersionInfo;
                break;
            case OpenvrIntegration::PerformanceHudMode::VersionInfo:
                _curPerfHudMode = OpenvrIntegration::PerformanceHudMode::Off;
                break;
        }
    ...
    /* Toggle through the debug hud stereo modes */
        ...
        switch(_curDebugHudStereoMode) {
            case OpenvrIntegration::DebugHudStereoMode::Off:
                _curDebugHudStereoMode = OpenvrIntegration::DebugHudStereoMode::Quad;
                break;
            case OpenvrIntegration::DebugHudStereoMode::Quad:
                _curDebugHudStereoMode = OpenvrIntegration::DebugHudStereoMode::QuadWithCrosshair;
                break;
            case OpenvrIntegration::DebugHudStereoMode::QuadWithCrosshair:
                _curDebugHudStereoMode = OpenvrIntegration::DebugHudStereoMode::CrosshairAtInfinity;
                break;
            case OpenvrIntegration::DebugHudStereoMode::CrosshairAtInfinity:
                _curDebugHudStereoMode = OpenvrIntegration::DebugHudStereoMode::Off;
                break;
        }
...
MAGNUM_APPLICATION_MAIN(Magnum::Examples::OpenvrExample)
